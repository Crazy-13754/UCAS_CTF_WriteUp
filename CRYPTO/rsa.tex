...
此时，利用性质 \ref{CRT性质倍数}，可以将 $x_i \equiv a_i(\bmod n_i)$简化为求$x_i' \equiv 1 (\bmod n_i)$ 且 $x_i = a_i \cdot x_i'$。令$k_i$为$n_1,n_2, \cdots, n_{i-1}, n_{i+1}, \cdots, n_r$的乘积，根据性质 \ref{CRT性质找数} 则有$x_i'$满足$k_i \cdot x_i' \equiv 1 (\bmod n_i)$，即求 $k_i$ 模 $n_i$ 的逆\upcite{中国剩余定理(CRT)}。
